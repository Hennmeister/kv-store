...
with open("./data/exp1_data.txt", "r") as data:
    ...
    for line in data.readlines()[1:-1]:
        line = line.split(",")
        ...
        put_throughput.append(1000 * temp_num_puts / temp_puts_microsec)
        get_throughput.append(1000 * temp_num_gets / temp_gets_microsec)
        scan_throughput.append(1000 * temp_num_scans / temp_scans_microsec)

    fig, (ax1, ax2, ax3) = plt.subplots(3)  
    fig.set_figheight(17)

    ax1.plot(x, put_throughput)
    ax1.set_title("Put Throughput (operations/msec)")
    ax1.set_xlabel("Inserted data (Bytes)")
    ax1.set_ylabel(y_label)

    ax2.plot(x, get_throughput)
    ax2.set_title("Get Throughput (operations/msec)")
    ax2.set_xlabel("Inserted data (Bytes)")
    ax2.set_ylabel(y_label)

    ax3.plot(x, scan_throughput)
    ax3.set_title("Scan Throughput (operations/msec)")
