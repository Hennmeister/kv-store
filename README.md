...
_Note that each header below is a link to the directory or source file containing relevant logic_

### [**KV-store get API**](./src/KVStore/SimpleKVStore.cpp)
...
### [**KV-Store put API**](./src/KVStore/SimpleKVStore.cpp)
...
### [**Memtable**](./src/KVStore/RedBlackMemtable.cpp)

We implement a memtable as a balanced binary search tree ([red-black tree](https://en.wikipedia.org/wiki/Red%E2%80%93black_tree) to be precise) supporting standard `put(key,val)`, `get(key)` and `scan(key1, key2)` methods. We also implement an inorder tree traversal that returns all the key-value pairs sorted by their keys, which will be helpful to dump the content into the corresponding SST (refer to next bullet point). See also [RedBlackTree](./src/RedBlackTree/RedBlackTree.cpp).

### [**SSTs (Sorted String Tables)**](./src/SortedArraySST)
...
### [**Database Open and Close API**](./src/KVStore/SimpleKVStore.cpp)
...
### [**Extendible Hash Buffer Pool**](./src/BufferPool/)
...
### [**Integration of Buffer with get**](./src/SimpleSSTFileManager.cpp)

To interact with disk, we use a [file manager](./include/SimpleSSTFileManager.h) as an abstraction. Entries are only placed in the buffer pool on gets. If a get to the buffer pool fails from within the file manager, the page is retrieved from disk and then placed into the buffer pool. See the get and scan functions for use of the cache.

### [**Shrink API**](./src/BufferPool/Directory.cpp)
...
### [**Eviction Policies**](./src/BufferPool/)

Two eviction policies, [`ClockBuffer`](./src/BufferPool/ClockBuffer/ClockBuffer.cpp) and [`LRUBuffer`](./src/BufferPool/LRUBuffer/LRUBuffer.cpp), are implemented as derived classes of `Directory` and located in the `./include/BufferPool` and `./src/BufferPool` directories. The use of inheritance instead of something like a strategy pattern makes the use of eviction-policy-specific metadata easier to handle.
...
### [**LSM Tree**](./src/LSMTreeManager.cpp)
...
### [**Updates**](./src/KVStore/SimpleKVStore.cpp)
...
### [**Deletes**](./src/KVStore/SimpleKVStore.cpp)
